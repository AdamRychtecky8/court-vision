...
Schema confirmed from `01_ingestion/verify_shots.py` output on 2026-05-30.*

The raw season CSVs are never committed to this repository — `data/raw/` is
gitignored and the files live only on local disk and in Google Cloud Storage,
where the Dataproc pipeline reads them from. This file is the single source of
truth for the schema, so the dataset is fully understandable without the files
present.
...
| Coverage | 2003-04 through 2024-25 NBA seasons (21 seasons) |
| File format | 21 CSVs, one per season, named `NBA_YYYY_Shots.csv` |
...
## Staging the Data into GCS

To reproduce the pipeline from scratch:

1. Download the 21 season CSVs from the source above into `data/raw/` locally
   (see `01_ingestion/ingest_shots.md` for the full download notes, including a
   workaround for networks that block Kaggle/GitHub downloads directly).

2. Create or reuse a GCS bucket, then upload the raw files:
...
4. Point `RAW_PATH` in `01_ingestion/verify_shots.py` and `02_processing/process_shots.py`
   (and the other `gs://` paths in `03_shot_model/` and `05_clustering/`) at your bucket,
   then run the pipeline as described in the root `README.md`.

Full GCP project/cluster setup is in `01_ingestion/setup_commands.md`.
